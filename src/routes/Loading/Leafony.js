...
function Leafony() {
    ...
    /**
     * Bluetoothリーフと接続する関数
     * @param message 接続完了時からleafonyに送るメッセージ
     * @return none
     */
    ...
    async function connectService(server) {
        ...
        // setInterval(sendCommand, 1000, sendMessage);  // 毎秒SND送信。1回でよくない？多分届かなかった時のため。
        setTimeout(sendCommand, 1000, sendMessage);
    }
    /**
    ...
    function handleData(event) {
        ...
        state.item14 = data[3];
        state.item15 = data[4];
        // ここでapp.jsのコールバックが実行される。すぐ下のonStateChangeCallback()は何に使うの？
